...
AmorApohele = len([1 for obj in MylistOfSB if ( (float(obj.PEq) > 1.017 and float(obj.PEq) <1.3 and float(obj.PEQ) < 1)) ])
...
ApolloApohele = len([1 for obj in MylistOfSB if ( (float(obj.PEq) < 1.017 and float(obj.PEa) >1 and float(obj.PEQ) < 1)) ])
print(f"Apollo Apohele presek: {ApolloApohele}")
                     
AtensApohele= len([1 for obj in MylistOfSB if ( (float(obj.PEa) < 1 and float(obj.PEQ) >0.983 and float(obj.PEQ) < 1)) ])
print(f"Atens Apohele presek: {AtensApohele}")

#### ima preklapanja u skupu:  Atens presek Apohele --- 18 komada
